...
api_key = 'get this from Twitter'

api_secret_key = 'get this from Twitter'

api_access_token = 'get this from Twitter'

api_secret_access_token = 'get this from Twitter'
...
def i_am_not_following(username, myself=YOURUSERID): #this is not working correctly and killing my rate limit
    #time.sleep(90)
    returnvalue = None
    try:
...
def block_followers(investigated_user, oldcursor=-1, myself=YOURUSERID):
    #if(i_am_not_following(investigated_user)):
    if(True):
        openfile = investigated_user + '.txt'
        f = open(openfile, 'a+')
        newcursor = oldcursor
        stillgoing = True
        count = 0
        try:
            while stillgoing:
                ...
                for user in userlist[2]:
                    #print('am i following... ' + user.screen_name)
                    #if stillgoing and i_am_not_following(user.screen_name):
                    if stillgoing:
                        blockeduser = api.CreateBlock(screen_name=user.screen_name)
            ...
            print(str(datetime.datetime.now()) + ' In block_followers method : probably ran into the rate limit ' + str(newcursor) + ' ' + investigated_user)
            time.sleep(120)
            print(e.args[0])
            if e.message and e.message == 'User not found.':
                print(str(datetime.datetime.now()) + ' In block_followers method : user not found block. Start wait. ' )
                time.sleep(90)
                print(str(datetime.datetime.now()) + ' In block_followers method : user not found block. End wait. ' )
                block_followers(investigated_user, newcursor)
            elif e.message and e.message == ' Rate limit exceeded':
                print(str(datetime.datetime.now()) + ' In block_followers method : rate limit block. Start wait. ' )
                time.sleep(90)
                print(str(datetime.datetime.now()) + ' In block_followers method : rate limit block. End wait. ' )
                block_followers(investigated_user, newcursor)
        blockeduser = api.CreateBlock(screen_name=investigated_user)
        f.close()
...
try:
    while True:
        #listed_user = testfile.readline().rstrip('\n')
        ...
        if not user_line:
            ...
            cursor = next_user[1]
            #block_followers(listed_user)
            block_followers(listed_user, cursor)
